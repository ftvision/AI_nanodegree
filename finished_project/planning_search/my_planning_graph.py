...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if prenodes.issubset(self.s_levels[level]):
                # the issubset usage is magic: nodes has a HASH value, and Python use hashtable
                # to implement set(), so even two PgNode may be constructed at different time
                # their hash value may be the same, therefore we can use `issubset` to test
                # whether action.prenodes is a subset of s_levels[level]
                self.a_levels[level].add(node)
                for s_node in self.s_levels[level]:
                    if s_node in prenodes:
                        s_node.children = node
                        node.parent = s_node
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            effnodes = a_node.effnodes
            self.s_levels[level].update(effnodes)
            for s_node in self.s_levels[level]:
                if s_node in effnodes:
                    s_node.parent = a_node
                    a_node.children = s_node
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        negation.update(set(a1.effect_add) & set(a2.precond_neg))
        negation.update(set(a1.effect_rem) & set(a2.precond_pos))
        negation.update(set(a2.effect_add) & set(a1.precond_neg))
        negation.update(set(a2.effect_rem) & set(a1.precond_pos))
        return len(negation) > 0
